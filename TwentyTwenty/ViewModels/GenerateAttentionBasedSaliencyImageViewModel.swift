...
final class GenerateAttentionBasedSaliencyImageViewModel: BaseModelDetailViewModel {
    ...
    var overlayColor: UIColor = UIColor(hue: 0.05, saturation: 1.0, brightness: 1.0, alpha: 1.0) {
        didSet {
    ...
    private func performAttentionSaliencyGeneration(on image: UIImage) async throws -> ([AttentionSaliency], CVPixelBuffer?) {
        ...
        let results = [AttentionSaliency(from: observation, index: 0, imageSize: image.size)]
